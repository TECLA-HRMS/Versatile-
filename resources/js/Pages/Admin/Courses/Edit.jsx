...
const ImagePreview = ({ file, url }) => {
    ...
    return (
        <div style={{ marginTop: '10px', borderRadius: '8px', overflow: 'hidden', border: '1px solid var(--border-color)', display: 'inline-block', background: '#f8fafc' }}>
            <img src={window.assets(src)} alt="Preview" style={{ maxHeight: '120px', maxWidth: '100%', display: 'block', objectFit: 'contain' }} />
        </div>
    );
};

export default function Edit({ course, programs }) {
    const { data, setData, post, processing, errors } = useForm({
        _method: 'put', program_id: course.program_id || '', name: course.name || '', slug: course.slug || '', short_desc: course.short_desc || '', description: course.description || '',
        thumbnail: '', banner_image: '', stats: course.stats || [], competencies: course.competencies || [], advantages: course.advantages || [],
        eligibility: course.eligibility || [], entrance_test: course.entrance_test || [], placements: course.placements || [], brochure_file: '',
        university_name: course.university_name || '', university_logo: '', duration: course.duration || '', sort_order: course.sort_order || 0, is_active: course.is_active !== undefined ? course.is_active : true,
    });

    const handleSubmit = (e) => { e.preventDefault(); post(`${window.AppAssetUrl || '/'}admin/courses/${course.id}`); };
                    ...
                    <div className="row">
                        ...
                        <div className="col-md-6">
                            <Field icon="ri-image-2-line" label="University Logo">
                                ...
                                <ImagePreview file={data.university_logo} url={course.university_logo} />
                            </Field>
                        </div>
                        <div className="col-md-6">
                            <Field icon="ri-camera-line" label="Course Thumbnail" hint="Recommended: 800×450px">
                                ...
                                <ImagePreview file={data.thumbnail} url={course.thumbnail} />
                            </Field>
                        </div>
                        <div className="col-md-6">
                            <Field icon="ri-image-line" label="Banner Background" hint="Recommended: 1920×600px">
                                ...
                                <ImagePreview file={data.banner_image} url={course.banner_image} />
                            </Field>
                        </div>
                        <div className="col-md-6">
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 0 40px', borderTop: '1px solid var(--border-color)', marginTop: 8 }}>
                    ...
                    <div className="d-flex gap-2">
                        ...
                        <button type="submit" className="btn-primary" disabled={processing}>
                            <i className="ri-check-line"></i> {processing ? 'Publishing...' : 'Update Course'}
                        </button>
                    </div>
                </div>
            </form>
